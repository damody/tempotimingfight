...
{
public:
	GetDirectXInput(HWND hWnd, HINSTANCE Instance);
	int CheckInputDevice(InputDevice input);
	int InputInit(void);
	int InputClose(void);
...
private:	
	...
	static POINT m_op;
	static POINT m_mouse;
	static KeyHandlerProc m_pKeyDownFuncs[256];
	static KeyHandlerProc m_pKeyUpFuncs[256];
	static KeyHandlerProc m_pKeyPressedFuncs[256];
	static char   m_keyboard_state[256];
};
