...
int BMPImg::Load(const wchar_t* szFilename)
{
	int iRet;
...
int BMPImg::GetFile(const wchar_t* szFilename)
{
	using namespace std;
	ifstream fIn;
...
Color24 BMPImg::GetPoint( int x, int y )
{
	int bytes_per_pixel = iBPP/8;
	unsigned char *point = GetPixel(x,y);
	Color24 c24;
...
void BMPImg::SetPoint( int x, int y, Color24 c24 )
{
	int bytes_per_pixel = iBPP/8;
	unsigned char *point = GetPixel(x,y);
	*point = c24.r;
